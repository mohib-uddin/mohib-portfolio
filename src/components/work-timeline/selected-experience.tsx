...
const SelectedExperience=({experience}:{experience:workExperienceType[]})=>{
    ...
    return(
        <div className={'flex w-full p-4 mt-4 m-auto gap-3 dark:text-white desktop:flex-row  flex-col'}>
            <div className={'flex overflow-x-auto flex-row laptop:flex-col border-l-2 border-l-black dark:border-l-white h-[fit-content] p-2'}>
                {experience.map((e,index)=>{
                    return(
                        <div key={index} className={'desktop:p-2'} onClick={()=>setSelectedExperience(e)}>
            ...
            <motion.div key={selectedExperience.name} initial={{y:-10,opacity:0}} animate={{y:10,opacity:1}} transition={{duration:1,ease:"easeIn"}} className={'w-full desktop:w-1/2 text-xl'}>
                ...
                {selectedExperience.desc&&Array.isArray(selectedExperience.desc)&&<ul className={'text-base dark:text-color__secondary p-2'}>
